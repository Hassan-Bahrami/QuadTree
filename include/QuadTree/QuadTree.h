...
public:
    ...
    std::vector<Point> kNearestWrapper(const Point& searchPoint, int maxCount, double maxDistance) {
        double sqMaxDistance = maxDistance * maxDistance;
        double furthestSqDistance = INFINITY; // Initialize to infinity
        std::vector<Point> found;
        int foundSoFar = 0; // Initialize foundSoFar to 0
...
private:
    ...
    std::vector<Point> kNearest(const Point& searchPoint, int maxCount, double sqMaxDistance, double furthestSqDistance, int foundSoFar, std::vector<Point>& found) const;
};
