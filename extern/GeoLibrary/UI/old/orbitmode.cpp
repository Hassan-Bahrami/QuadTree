...
//  vcg::trackutils::DrawSphereIcon(tb,false);
...
{
  Geo::Point3<float> hitOld = HitSphere (tb, tb->last_point);//Geo::Point3<float>(tb->last_point.X(),
                                                             //   tb->last_point.Y(),
                                                             //   tb->last_point.Z()));
  Geo::Point3<float> hitNew = HitSphere (tb, new_point);
//  tb->Hits.push_back (hitNew);
  //Geo::Point3<float> center = Geo::Point3<float>(tb->center.X(),tb->center.Y(),tb->center.Z());
...
//  float phi = std::max(vcg::Angle(Geo::ToVCG(hitNew - tb->center),Geo::ToVCG(hitOld - tb->center)),
...
void Geo::SphereMode::Draw(Geo::Trackball * tb){
  //vcg::trackutils::DrawSphereIcon(tb,true );
...
//  Geo::Point3<float> hitOld = HitViewPlane (tb, Geo::Point3<float>(tb->last_point.X(),
//                                                                   tb->last_point.Y(),
//                                                                   tb->last_point.Z()));
...
//  vcg::trackutils::DrawSphereIcon(tb,true);
//  vcg::trackutils::DrawSphereAxis(tb);
  Geo::DrawSphereIcon(tb,true);
