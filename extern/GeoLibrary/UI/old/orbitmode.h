...
//vcg::Point3<ScalarType> ToVCG(const Geo::Point3<ScalarType> p)
//{
//    return vcg::Point3<ScalarType> (p.X,p.Y,p.Z);
//}

//template <class ScalarType>
//Geo::Point3<ScalarType> ToGeo(const vcg::Point3<ScalarType> p)
//{
//    return Geo::Point3<ScalarType> (p.X(),p.Y(),p.Z());
//}
...
//    void GetView()
//    {
//        vcg::Matrix44<float> ProjVcg;
//        GLDraw::glGetv(GL_PROJECTION_MATRIX,ProjVcg);
//        ProjVcg.ToEigenMatrix(proj);

//        vcg::Matrix44<float> ModelVcg;
//        GLDraw::glGetv(GL_MODELVIEW_MATRIX,ModelVcg);
//        ModelVcg.ToEigenMatrix(model);
...
//        else isOrtho = true;
    ...
    //  vcg::Point3<T> Project(const vcg::Point3<T> &p) const;
    //  vcg::Point3<T> UnProject(const vcg::Point3<T> &p) const;
//    vcg::Point3<T> ViewPoint() const
//    {
//        //if(isOrtho) return vcg::Inverse(model)* vcg::Point3<T>(0, 0, 3);
//        //if(isOrtho) return vcg::Inverse(model)* vcg::Point3<T>(0, 0, 3);

//        //Eigen::Matrix4f invModel=model.inverse();
//        vcg::Point3<T> Ret(-model(0,3),-model(1,3),-model(2,3));
//        Ret*=model(3,3);
//        return Ret;
////        vcg::Matrix44f modelvcg;
////        modelvcg.FromEigenMatrix(model);
////        return vcg::Inverse(modelvcg)* vcg::Point3<T>(0, 0, 0);
//    }
    ...
    //  vcg::Plane3<T> ViewPlaneFromModel(const vcg::Point3<T> &p);

    //  /// Return the line of view passing through point P.
    //  vcg::Line3<T> ViewLineFromModel(const vcg::Point3<T> &p);

//    //  /// Return the line passing through the point p and the observer.
//    vcg::Line3<T> ViewLineFromWindow(const vcg::Point3<T> &p)
//    {
//        vcg::Line3<T> line;  // plane perpedicular to view direction and passing through manip center
//        vcg::Point3<T> vp=ViewPoint();
//        vcg::Point3<T> pp=UnProject(p);
...
//    }
    ...
    {
        //Geo::Line3<T> line;  // plane perpedicular to view direction and passing through manip center
...
//    vcg::Point3<T> UnProject(const vcg::Point3<T> &p) const {
//      vcg::Point3<T> s = WindowCoordToNormDevCoord(p);
//      vcg::Matrix44f invVCG;
//      invVCG.FromEigenMatrix(inverse);
//      s =  invVCG * s ;
//      return s;
//    }
...
//    vcg::Point3<T> WindowCoordToNormDevCoord(const vcg::Point3<T> &p) const {
//      vcg::Point3<T> a;
...
//    }
    ...
    //  vcg::Point3<T> NormDevCoordToWindowCoord(const vcg::Point3<T> &p) const;
...
//    vcg::Matrix44<T> matrix;
//    vcg::Matrix44<T> inverse;
...
public:
  ...
  */
    virtual void Apply (Trackball * trackball, Geo::Point3<float> new_point);
    //  virtual void Apply (Trackball * trackball, Geo::Point3<float> new_point);
    /*!
